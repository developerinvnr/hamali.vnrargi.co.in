...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	if($_POST['acn']=="save")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("insert into $tablename($fields) values($values)"))
			{
				...
				unset($flag);				
				unset($msgErr);				
				$_SESSION['success']	=	"Unit name added successfully!";
				echo '<script>document.location.href="./mainindex.php?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
			{
				$_SESSION['warning']	=	"Unit name already exist. Please try again!";
				echo '<script>document.location.href="./mainindex.php?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
		}
		...
		if($flag==0)
		{
			...
			while($row_fields=mysqli_fetch_assoc($rs_fields))
			{
				...
				if($up==1)
				{
					...
					{
						$updatefields.=	"".$row_fields['fieldname']."='".date('Y\-m\-d',$_POST[$row_fields['fieldname']])."'";
					}
			...
			if($dbconnection->firequery("update $tablename set $updatefields where ".$_REQUEST['pk']."=".trim(decryptvalue($_POST[$_REQUEST['pk']])).""))
			{
				...
				unset($row_fields);				
				unset($up);				
				$_SESSION['success']	=	"Unit name updated successfully!";
				echo '<script>document.location.href="./mainindex.php?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
	...
	if($_POST['acn']=="delete")
	{
		if(!$dbconnection->isRecordExist("select * from testparameter_tbl where unitid=".trim(decryptvalue($_POST[$_REQUEST['pk']])).""))
		{
			if($dbconnection->firequery("delete from $tablename where ".$_REQUEST['pk']."=".trim(decryptvalue($_POST[$_REQUEST['pk']])).""))
			{ 
				$_SESSION['success']	=	"Unit detail deleted successfully!";
				echo '<script>document.location.href="./mainindex.php?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
		}
		...
		{
			$_SESSION['success']	=	"Unit detail already attahced with some parameter. So it can not be deleted!";
			echo '<script>document.location.href="./mainindex.php?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
			exit;
		}
	}
		...
		<form name="frm" id="frm" action="#" method="post">
			...
			?>
				<div class="col-sm-3">
					...
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Add Unit Name</button>
					<?php
					}
					else
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Update Unit Name</button>
					<?php
					}
					?>
				</div>
	...
	<div class="table-responsive">
		<table class="table table-bordered" id="tablerecords">
			<thead>
				...
				<tr>
					<td style="padding:3px; width:25px;">SNO</td>
					<td style="padding:3px;">UNIT NAME</td>
					<td style="padding:3px; text-align:center; width:25px;"><i class="fa fa-edit"></i></td>
					<td style="padding:3px; text-align:center; width:25px;"><i class="fa fa-remove"></i></td>
				</tr>
