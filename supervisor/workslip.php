		...
		<table class="table table-bordered table-hover" id="tablerecords">
			<thead>
				<tr>
					...
					<div>
						From Date &nbsp;
						<input type="date" name="from_date" id="from_date" value="<?php echo date('Y-m-d',strtotime('-1 month',strtotime(date('Y-m-d')))); ?>" />
					</div>
					</th>
					<th colspan="2">
    					<div>
					...
					<th colspan="3">
					<div>
						Payment Status &nbsp;<select name="payment_status" id="payment_status" class="form-control" onchange="GetLoadDefault()" tabindex="<?php echo $t++;?>">
...
<script>
	...
	$(document).on('change','#from_date,#to_date,#payment_status',function(){
		LoadWorkSlip();
	});
