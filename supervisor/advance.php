...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	if($_POST['acn']=="save")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("insert into advance_tbl(groupname,advancedate,amount,paymentmode,cdno,bankname,department,creationdate,supervisorid,remark) values(".$_POST['groupname'].",'".date('Y\-m\-d',strtotime($_POST['advancedate']))."',".doubleval($_POST['amount']).",'".$_POST['paymentmode']."','".$_POST['cdno']."','".$_POST['bankname']."',".$_SESSION['supervisordetail'][0]['departmentid'].",'".date('Y\-m\-d H:i:s')."',".$_SESSION['supervisordetail'][0]['sessionid'].",'".$_POST['remark']."')"))
			{
				unset($fields);
				unset($values);				
				unset($flag);				
		...
		<form name="frm" id="frm" action="#" method="post">
			...
			while($row=mysqli_fetch_assoc($rs_sel))
			{
				...
				<div class="col-sm-<?php echo $row['columnsize'];?>">
					...
					{
					?>
					<input type="date" class="form-control" name="<?php echo $row['fieldname'];?>" id="<?php echo $row['fieldid'];?>" value="<?php if($_POST['advancedate']=="") echo date('Y\-m\-d'); else echo date('Y\-m\-d',strtotime($_POST['advancedate']));?>" placeholder="<?php echo ucwords($row['fieldcaption']);?>" onKeyPress="return OnKeyPress(this, event)" <?php echo $row['isrequired'];?> tabindex="<?php echo $t++;?>" <?php echo $row['autofocus'];?> autocomplete="<?php echo $row['autocomplete'];?>" style="text-transform:uppercase;"/>
					<?php
					}
		...
		<table class="table table-bordered table-hover" id="tablerecords">
			<thead>
				...
				<tr>
					...
					<th style="padding:3px;">PAYMENT MODE</th>					
					<th style="padding:3px;">CHEQUE/DD NUMBER</th>					
					<th style="padding:3px;">BANK NAME</th>										
