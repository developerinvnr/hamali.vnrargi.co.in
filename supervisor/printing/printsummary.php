...
<center>
<body>
<h4>Ledger Summary For <?php echo $dbconnection->getField("center_tbl","centername","centerid=".$_REQUEST['centerid']."");?> From <?php echo date('d\-m\-Y',strtotime($frmdate));?> To <?php echo date('d\-m\-Y',strtotime($todate));?></h4>
...
<?php
	...
	?>
		<tr>
			...
			<?php
			$received	=	$dbconnection->getField("staffledger_view","sum(payingamount)","date(paymentdate)<'".date('Y\-m\-d',strtotime($frmdate))."' and centerid=".$_REQUEST['centerid']."");
			$paid		=	$dbconnection->getField("staffledger_view","sum(receivingamount)","date(paymentdate)<'".date('Y\-m\-d',strtotime($frmdate))."' and centerid=".$_REQUEST['centerid']."");			
		...
		$rs_led	=	$dbconnection->firequery("select sum(paid) as paid,sum(received) as received,paymentdate,centerid from staffledger_summary where centerid=".$_REQUEST['centerid']." and date(paymentdate)='".date('Y\-m\-d',strtotime($frmdate))."' group by date(paymentdate)");
	...
	<tr>
		...
		<?php
		$received	=	$dbconnection->getField("staffledger_view","sum(payingamount)","date(paymentdate)>'".date('Y\-m\-d',strtotime($todate))."' and centerid=".$_SESSION['centerdetail'][0]['centerid']."");
		$paid		=	$dbconnection->getField("staffledger_view","sum(receivingamount)","date(paymentdate)>'".date('Y\-m\-d',strtotime($todate))."' and centerid=".$_SESSION['centerdetail'][0]['centerid']."");			
