	...
	if($_POST['acn']=="save")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("insert into workslip_tbl(workslipnumber,workslipdate,groupnumber,groupname,remark,workslipamount,creationdate,supervisorid) values('".$slipno."','".date('Y\-m\-d H:i:s',strtotime($_POST['workslipdate']))."','".$_POST['groupnumber']."','".$_POST['groupname']."','".$_POST['rem']."',".doubleval($_POST['totalvalue']).",'".date('Y\-m\-d H:i:s')."',".$_SESSION['supervisordetail'][0]['sessionid'].")"))
			{
				...
				echo '<script>document.location.href="./vnr_mainindex?m='.encrypt("workslip").'&p='.encrypt("workslipextended").'";</script>';
				exit;
			}
			else
			{
			...
			<table class="table table-bordered table-hover" id="tablerecords">
				<thead>				
					<tr>
						...
						<td style="padding:3px; width:200px;">Material</td>
						<td style="padding:3px; width:200px;">Product/Method</td>												
...
<script>
	...
	function LoadWorkSlip()
	{
		...
		$(".loader").show();
		$.post("ajaxpages/workslipextendedlist.php",
		{
	...
	{
		$(".loader").show();
		$.post("ajaxpages/extendedrecord.php",
		{
			processname: "add",
		},
		function(data, status){
