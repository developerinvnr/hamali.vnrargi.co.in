...
if(isset($_SESSION['supervisordetail'][0]['sessionid']))
{
	...
	$hgname			=	$_POST['hgname'];
	if($hgname=="")
	$query	=	"select a.*,b.groupname from paymentslip_tbl a left join hamaligroup_tbl b on b.hgid=a.groupnumber where a.supervisorid=".$_SESSION['supervisordetail'][0]['sessionid']." ";
	else
	$query	=	"select a.*,b.groupname from paymentslip_tbl a left join hamaligroup_tbl b on b.hgid=a.groupnumber where a.supervisorid=".$_SESSION['supervisordetail'][0]['sessionid']." and a.groupnumber=".$hgname." ";
	$query1=$query;
...
<tr>
	...
	<td align="center"><?php echo date('d\-m\-Y h:i A',strtotime($row['payslipdate'])); $cols++;?></td>
	<td align="center"><?php echo $row['payslipnumber']; $cols++;?></td>	
	<td><?php echo $row['groupname']; $cols++;?></td>		
	<td><?php echo $row['remark']; $cols++;?></td>	
	<td align="center"><i class="fa fa-inr"></i> <?php echo $row['totalamount']; $cols++;?></td>			
	<td align="center"><i class="fa fa-inr"></i> <?php echo $balance=$row['totalamount']-$paid; $cols++;?></td>
