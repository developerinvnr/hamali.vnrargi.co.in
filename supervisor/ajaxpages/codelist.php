...
<tr>
	<td style="padding:0px; width:85px;">
		<input type="text" name="workcode<?php echo $i;?>" id="workcode<?php echo $i;?>" value="<?php echo $det['workcode'];?>" readonly tabindex="<?php echo $t++;?>" onKeyPress="return OnKeyPress(this, event)" style="padding-left:2px; padding-top:0px; padding-bottom:0px; padding-right:0px; height:25px; font-size:12px; width:100%;" autocomplete="off" class="slipdetail" title="<?php echo $i;?>" />
	</td>
	<td style="padding:0px; width:85px;">
		...
		<input type="hidden" name="rate<?php echo $i;?>" id="rate<?php echo $i;?>" value="<?php echo $det['rate'];?>" />
		<input type="hidden" name="remark<?php echo $i;?>" id="remark<?php echo $i;?>" value="<?php echo $det['narration'];?>" />
		
		<input type="hidden" name="detailid<?php echo $i;?>" id="detailid<?php echo $i;?>" value="<?php echo $det['detailid'];?>" />
		<input type="hidden" name="workslipid<?php echo $i;?>" id="workslipid<?php echo $i;?>" value="<?php echo $det['workslipid'];?>" />
		<input type="hidden" name="total<?php echo $i;?>" id="total<?php echo $i;?>" value="<?php echo $det['total'];?>" />
		<input type="hidden" name="ind<?php echo $i;?>" id="ind<?php echo $i;?>" value="<?php echo $i;?>" />
	</td>
	...
	<button type="button" class="btn" style="padding-left:0px; padding-top:0px; padding-right:0px; padding-bottom:0px; border:none; color:#FFFFFF; font-size:12px; font-weight:bold; width:100px; height:25px;" onclick="RemoveRecord(<?php echo $det['detailid'];?>,<?php echo $det['workslipid'];?>,<?php echo $det['total'];?>)">Delete</button> 
...
<tr>
	<td colspan="7" style="text-align:right;">
		<button type="button" class="btn" style="margin-right:5px;" onclick="UpdateWorkSlipDetail()">UPDATE WORK SLIP</button>
