...
if(isset($_SESSION['supervisordetail'][0]['sessionid']))
{
	...
	$gpno			=	$_POST['gpno'];
	if($gpno=="")
	$query	=	"select * from workslip_tbl where supervisorid=".$_SESSION['supervisordetail'][0]['sessionid']." order by workslipdate desc ";
	else
	$query	=	"select * from workslip_tbl where supervisorid=".$_SESSION['supervisordetail'][0]['sessionid']." and groupnumber=".$gpno." order by workslipdate desc ";
...
<tr>
	...
	{
	?>
	<input type="checkbox" class="slips" onclick="SelectSlip()" name="slipid<?php echo $i;?>" id="slipid<?php echo $i;?>" value="<?php echo $row['workslipid'];?>" style="float:left;" />
	<?php
	}
	?>	
	</td>
...
<tr id="rec<?php echo $i;?>" style="display:none;" class="bg-ingo text-white wrec"><td id="recd<?php echo $i;?>" colspan="9" style="width:100%;"></td></tr>
<?php
}
if($j>0)
{
