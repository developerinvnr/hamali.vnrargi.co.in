		...
		<table class="table table-bordered table-hover" id="tablerecords">
			<thead>
				<tr>
					<th colspan="12">
					<div id="tablehead">
						<select name="centername" id="centername" class="selectbx" tabindex="<?php echo $t++;?>" onchange="GetStaff(this.value,'staffname')">
						...
						<button class="btn btn-info" onclick="LoadDefault()" style="padding:0px 5px; height:30px;">Search</button>										
					</div>
					</th>
				</tr>
...
<script>
	...
	function GetStaff(centerid,htmlid)
	{
	  var xhttp;    
	  ...
	  xhttp.open("POST", "ajaxpages/staffname.php?q="+centerid, true);
	  xhttp.send();
	}


	function SetPrice(customerid,ind,centerid)
	{
		var referredbyid	=	document.getElementById("referredbyid"+ind).value;				
		var m				=	document.getElementById("m").value;
		var p				=	document.getElementById("p").value;				
		if(referredbyid!="")
		{
			$(".loader").show();		
			$.post("ajaxpages/setprice.php",
			{
			...
			function(data, status){
				$(".loader").hide();
				$(".msg"+ind).css("display","");
				$(".msg"+ind).html("RECORD UPDATED SUCCESSFULLY!");
				setTimeout(function(){ $(".msg"+ind).css("display","none"); LoadDefaul(); },3000);
