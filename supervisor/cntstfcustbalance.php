		...
		<table class="table table-bordered table-hover" id="tablerecords">
			<thead>
				<tr>
					<th colspan="9">
					<div id="tablehead">
						<select name="cntname" id="cntname" class="selectbx" onchange="LoadDefault()" tabindex="<?php echo $t++;?>">
						...
						while($roo=mysqli_fetch_assoc($rs_sel))
						{
						?>
						<option value="<?php echo $roo['centerid'];?>"><?php echo $roo['centername'];?></option>
						<?php
						}
						?>
						</select>
						&nbsp;&nbsp;&nbsp;&nbsp;
						From : <input type="date" name="frmdate" id="frmdate" class="selectbx" style="padding:0px;" value="<?php echo date('Y\-m\-01');?>" onchange="LoadDefault()" />
						To : <input type="date" name="todate" id="todate" class="selectbx" style="padding:0px;" value="<?php echo date('Y\-m\-d');?>" onchange="LoadDefault()" />						
					</div>
					</th>
				</tr>
...
<script>
	...
	function LoadDefault()
	{
		...
		$(".loader").show();		
		$.post("ajaxpages/cntstfcustbalance.php",
			{
				processname: "",
				cntname:cntname,
				frmdate:frmdate,
