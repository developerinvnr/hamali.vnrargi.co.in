			...
			<thead>
				<tr>
					...
					<div id="tablehead">
						...
						<?php
						$rs_sel	=	$dbconnection->firequery("select * from supervisor_tbl where franchisename=".$_SESSION['franchisedetail'][0]['sessionid']." order by supervisorname");
						while($frn=mysqli_fetch_assoc($rs_sel))
						{
						?>
						<option value="<?php echo $frn['supervisorid'];?>"><?php echo strtoupper($frn['supervisorname']);?></option>
						<?php
						}
						?>
						</select>
						<select name="centername" id="centername" class="selectbx" onchange="LoadDefault()">
...
<script>
	...
	function LoadDefault()
	{
		...
		$(".loader").show();		
		$.post("ajaxpages/receivelist.php",
			{
				processname: "",
