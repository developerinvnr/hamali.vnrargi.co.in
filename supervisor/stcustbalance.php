		...
		<table class="table table-bordered table-hover" id="tablerecords">
			<thead>
				<tr>
					...
					<div id="tablehead">
					<?php
					$rs_sel	=	$dbconnection->firequery("select * from staff_tbl where franchisename=".$_SESSION['franchisedetail'][0]['sessionid']." order by staffname");
					?>
					...
					{
					?>
					<option value="<?php echo $roo['staffid'];?>"><?php echo ucwords($roo['staffname']);?></option>
					<?php
					}
					unset($rs_sel);
					unset($roo);
...
<script>
	...
	function LoadDefault()
	{
		...
		$(".loader").show();		
		$.post("ajaxpages/stcustbalance.php",
			{
				processname: "",
				staffname:staffname,
				frmdate:frmdate,
