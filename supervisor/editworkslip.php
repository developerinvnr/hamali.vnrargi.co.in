			...
			<table class="table table-bordered table-hover" id="tablerecords">
				<thead>
					<tr>
						...
						<td style="padding:0px;"><input type="text" name="rem1" id="rem1" placeholder="Remark 1" style="padding-left:2px; padding-top:0px; padding-bottom:0px; width:100%;; padding-right:0px; height:25px; font-size:12px;" tabindex="<?php echo $t++;?>" onKeyPress="return OnKeyPress(this, event)" /></td>												

						<td style="padding:0px;"><input type="text" name="rem2" id="rem2" placeholder="Remark 2" style="padding-left:2px; padding-top:0px; padding-bottom:0px; width:100%;; padding-right:0px; height:25px; font-size:12px;" tabindex="<?php echo $t++;?>" onKeyPress="return OnKeyPress(this, event)" /></td>												
					...
					<tr>
						...
						<td style="padding:0px; font-weight:600; text-align:center; width:25px;">
							<button type="button" class="btn" disabled="disabled" style="padding-left:0px; padding-top:0px; padding-right:0px; padding-bottom:0px; border:none; color:#FFFFFF; font-size:12px; font-weight:bold; width:100px; height:25px;">Delete</button> 
						</td>						
...
function UpdateWorkSlipDetail()
{
	bootbox.confirm("Do you want to update this record", function(result)
	{
		if(result)
		{	
			$(".slipdetail").each(function(){		
				var total		=	document.getElementById("total"+this.title).value;
				var ind			=	document.getElementById("ind"+this.title).value;
				var workslipid	=	document.getElementById("workslipid"+this.title).value;
				var detailid	=	document.getElementById("detailid"+this.title).value;
				
				var workcode	=	document.getElementById("workcode"+this.title).value;
				var quantity	=	document.getElementById("quantity"+this.title).value;
				var rate		=	document.getElementById("rate"+this.title).value;
				var remark		=	document.getElementById("remark"+this.title).value;
				var rem1		=	document.getElementById("rem1"+this.title).value;
				var rem2		=	document.getElementById("rem2"+this.title).value;
				var narration	=	document.getElementById("narration"+this.title).value;
				UpdateWorkSlip(detailid,workslipid,ind,total,rate);
			});
		}
	});
}
function UpdateWorkSlip(did,slipid,ind,total,rate)
{
	$("#putmsg").html("");
	$("#deleterecord").hide();				
	$("#warningputmsg").html("");
	...
	$.post("ajaxpages/updaterecord.php",
	{
		slipid:slipid,
		did:did,
		total:total,
