...
<?php
$predates	=	date('Y\-m\-d H:i:s',strtotime('-30 days'));
?>
			
<div class="col-sm-3">
<label id="lab">From Date<label id="req">&nbsp;</label></label>
<input type="datetime-local" name="frmdate" id="frmdate" class="form-control" value="<?php echo date('Y\-m\-d',strtotime($predates)).'T'.date('H:i',strtotime($predates));?>">
</div>

<div class="col-sm-3">
<label id="lab">To Date<label id="req">&nbsp;</label></label>
<input type="datetime-local" name="todate" id="todate" class="form-control" value="<?php echo date('Y\-m\-d').'T'.date('H:i');?>">
</div>
...
<?php
$rs_dep	=	$dbconnection->firequery("select * from supervisor_tbl order by firstname");
$val="";
$i=0;
while($dep=mysqli_fetch_assoc($rs_dep))
{
...
<?php
$rs_dep	=	$dbconnection->firequery("select * from hamaligroup_tbl order by groupname");
while($dep=mysqli_fetch_assoc($rs_dep))
{
?>
...
<select class="form-control multiselect" name="workcode[]" id="workcode" multiple="multiple" onchange="AddWorkCode()">
<?php
$rs_dep	=	$dbconnection->firequery("select * from workcode_master order by workcode");
while($dep=mysqli_fetch_assoc($rs_dep))
{
?>
	<option value="<?php echo $dep['workcode'];?>">[<?php echo $dep['workcode'];?>] <?php echo $dep['defaultnarration'];?></option>
<?php
}
?>
</select>
<input type="hidden" name="workcodes" id="workcodes" value="<?php echo $val;?>">
</div>
...
<script>
	...
	function LoadDefault()
	{
		...
		var workcodes		=	document.getElementById("workcodes").value;
		var m				=	document.getElementById("m").value;
		var p				=	document.getElementById("p").value;				
		var pk				=	document.getElementById("pk").value;						
		$(".loader").show();		
		$.post("ajaxpages/workreportnew.php",
		{
			frmdate:frmdate,
			todate:todate,
			locations:locations,
