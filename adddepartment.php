...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	if($_POST['acn']=="save")
	{
		...
		{
			$flag++;
			$msgErr.=	"<li class='text-warning'>Please enter department name</li>";
		}
		if($flag==0)
		{
			$dbconnection->firequery("insert into department_tbl(departmentname,creationdate,addedby) values('".strtoupper($_POST['departmentname'])."','".date('Y\-m\-d H:i:s')."',".$_SESSION['datadetail'][0]['sessionid'].")");
			$_SESSION['success']	=	"Section name added successfully!";
			echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'&map=m";</script>';
			exit;			
		}
	}
	if($_POST['acn']=="update")
	{
		...
		if($flag==0)
		{
			$dbconnection->firequery("update department_tbl set departmentname='".strtoupper($_POST['departmentname'])."' where departmentid=".trim(decryptvalue($_POST[$_REQUEST['pk']]))."");
			$_SESSION['success']	=	"Section name updated successfully!";
			echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'&map=m";</script>';
			exit;			
		}
	}
	if($_POST['acn']=="delete")
	{
		if($dbconnection->firequery("delete from department_tbl where ".$_REQUEST['pk']."=".trim(decryptvalue($_POST[$_REQUEST['pk']])).""))
		{ 
			$_SESSION['success']	=	"Section detail deleted successfully!";
...
else
{
	...
	if($_REQUEST['pk']!="")
	{
		$rs_mast	=	$dbconnection->firequery("select * from department_tbl where ".$pk."=".trim(decryptvalue($_REQUEST['pk']))."");
		while($rowmast=mysqli_fetch_assoc($rs_mast))
		{
			$_POST['departmentname']	=	$rowmast['departmentname'];
		}
		unset($rs_mast);
		unset($rowmast);
		$_POST['acn']	=	"update";		
...
<div class="tabbable">
		<ul class="nav nav-tabs padding-18">
			<li class="active"><a data-toggle="tab" href="#home"><i class="green ace-icon fa fa-plus-circle bigger-120" style="vertical-align:bottom;"></i> Add Section</a></li>	
			<li><a data-toggle="tab" href="#feed"><i class="orange ace-icon fa fa-list bigger-120" style="vertical-align:bottom;"></i> Section List</a></li>
		</ul>
...
<form name="frm" id="frm" action="#" method="post">
			...
			while($row=mysqli_fetch_assoc($rs_sel))
			{
				...
				<div class="col-sm-<?php echo $row['columnsize'];?>">
					...
					{
					?>
					<textarea class="form-control" name="<?php echo $row['fieldname'];?>" id="<?php echo $row['fieldid'];?>" placeholder="<?php echo ucwords($row['fieldcaption']);?>" onKeyPress="<?php echo $row['onkeypress'];?>" <?php echo $row['isrequired'];?> tabindex="<?php echo $t;?>" <?php echo $row['autofocus'];?> autocomplete="<?php echo $row['autocomplete'];?>"><?php echo $_POST['fieldname'];?></textarea>
					<?php
					}
					else if($row['fieldtype']=="select")
					{
					?>
					<select class="form-control" name="<?php echo $row['fieldname'];?>" id="<?php echo $row['fieldid'];?>" onKeyPress="<?php echo $row['onkeypress'];?>" <?php echo $row['isrequired'];?> <?php echo $row['autofocus'];?> tabindex="<?php echo $t;?>">
					<option value="">--Select Center Name--</option>
					<?php
					$centernames	=	explode(",",$_POST['centername']);
					$rs_cen	=	$dbconnection->firequery("select * from center_tbl order by centername");
					while($cen=mysqli_fetch_assoc($rs_cen))
					{
					?>
					<option value="<?php echo $cen['centerid'];?>" <?php if(in_array($cen['centerid'],$centernames)) echo "selected";?>><?php echo $cen['centername'];?></option>
					<?php
					}
					?>
					</select>
			...
			?>
				<div class="col-sm-3">
					...
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t;?>">Add Section</button>
					<?php
					}
					else
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t;?>">Update Section</button>
					<?php
					}
					?>
				</div>
		...
		<table class="table table-bordered table-hover" id="tablerecords">
			<thead>
				...
				<tr>
					...
					<th style="padding:3px;">Section Name</th>
...
<script>
	...
	function UpdateData(v,d)
	{
		var testname	=	document.getElementById("testname"+v).value;
		var testamount	=	document.getElementById("testamount"+v).value;		
		var cpt			=	document.getElementById("cpt"+v).value;				

		$(".loader").show();		
		$.post("ajaxpages/updatetest.php",
			{
				testname:testname,
			...
			function(data, status){
				...
				if(data=="success")
				{
					bootbox.alert("Test detail updated successfully");	
					return false;
				}
				if(data=="error")
				{
