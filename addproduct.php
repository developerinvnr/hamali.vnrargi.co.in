...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	if($_POST['acn']=="save")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("insert into $tablename($fields) values($values)"))
			{
				...
				$unit	=	$dbconnection->getField("unit_tbl","unitname","unitid=".$_POST['unitname']."");
				$dbconnection->firequery("update product_tbl set unit='".$unit."' where productid=".$lid."");
				unset($fields);
				unset($values);				
				unset($flag);				
				unset($msgErr);			
				$_SESSION['success']	=	"Product/method name added successfully!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
			{
				$_SESSION['warning']	=	"Product/method, operator and unit combination is already available. Please try again!";
			}
		}
	}
	if($_POST['acn']=="update")
	{
		...
		if($flag==0)
		{
			...
			{
				$unit	=	$dbconnection->getField("unit_tbl","unitname","unitid=".$_POST['unitname']."");
				$dbconnection->firequery("update product_tbl set unit='".$unit."' where productid=".trim(decryptvalue($_POST[$_REQUEST['pk']]))."");			
				unset($updatefields);
				unset($rs_fields);
				unset($row_fields);				
				unset($up);
				$_SESSION['success']	=	"Product/method name updated successfully!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
	...
	if($_POST['acn']=="delete")
	{
		...
		{ 
			$_SESSION['success']	=	"Material name deleted successfully!";
			echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
			exit;
		}
		else
...
<div class="tabbable">
		<ul class="nav nav-tabs padding-18">
			<li class="active"><a data-toggle="tab" href="#home"><i class="green ace-icon fa fa-plus-circle bigger-120" style="vertical-align:bottom;"></i> Add Product/Method</a></li>	
			<li><a data-toggle="tab" href="#feed"><i class="orange ace-icon fa fa-list bigger-120" style="vertical-align:bottom;"></i> Product/Method Name List</a></li>
		</ul>
		...
		<form name="frm" id="frm" action="#" method="post">
			...
			while($row=mysqli_fetch_assoc($rs_sel))
			{
				...
				<div class="col-sm-<?php echo $row['columnsize'];?>">
					...
					{
					?>
					<select class="form-control" name="<?php echo $row['fieldname'];?>" id="<?php echo $row['fieldid'];?>" placeholder="<?php echo ucwords($row['fieldcaption']);?>" <?php echo $row['isrequired'];?> tabindex="<?php echo $t++;?>" onKeyPress="return OnKeyPress(this, event)">
			...
			?>
				<div class="col-sm-3">
					...
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Add Product/Method</button>
					<?php
					}
					else
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Update Product/Method</button>
					<?php
					}
					?>
				</div>
	...
	<div class="table-responsive">
		<table class="table table-bordered table-hover" id="tablerecords">
			<thead>
				...
				<tr>
					<th style="padding:3px; text-align:center; width:120px;">Product Code</th>
					<th style="padding:3px;">Product/Method Name</th>
