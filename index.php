...
if($_SESSION['datadetail'][0]['sessionid']!="" && $_SESSION['datadetail'][0]['authtype']=="ADMIN")
{
...
if($_SERVER['REQUEST_METHOD']=="POST")
{
    ...
    //echo 'aa='.decrypt('Xhwtzy23p8Ld52f9Suu5Ow_EQUALS__EQUALS_').' <Br>';
    //echo 'bb='.encrypt($_POST['password']);
	...
	if($_POST['acn']=="login")
	{
		...
		if($flag==0)
		{
		  //  echo encrypt($_POST['password']);
			$rs_sel		=	$dbconnection->firequery("select * from user_tbl where username='".$_POST['username']."' and password='".encrypt($_POST['password'])."'");
			$i=0;
			while($ro=mysqli_fetch_assoc($rs_sel))
			{
				...
				$loginname	=	$ro['username'];
				$logintype	=	$ro['usertype'];
				$pass		=	$ro['password'];
				$locations	=	$ro['location'];
				$utype		=	$ro['userstype'];
			}
			if($i>0)
			{
				$_SESSION['datadetail']=array();
				$_SESSION['datadetail'][0]['sessionid']=$userid;
				$_SESSION['datadetail'][0]['authname']=$authname;
				$_SESSION['datadetail'][0]['authid']=$loginname;
				$_SESSION['datadetail'][0]['authtype']=$logintype;
				$_SESSION['datadetail'][0]['pass']=$pass;
				$_SESSION['datadetail'][0]['loca']=$locations;
				$_SESSION['datadetail'][0]['ispayment']=$utype;
				unset($msgErr);
				unset($flag);
				$dbconnection->firequery("update user_tbl set lastlogin='".date('Y\-m\-d H:i:s')."' where userid=".$userid."");
				$dbconnection->firequery("insert into login_history(userid,usertype,logintime) values(".$userid.",'ADMIN','".date('Y\-m\-d H:i:s')."')");
				$_SESSION['successmessage']	=	"Dear $authname!, Welcome to Application Admin Panel. Thanks!";
				echo '<script>document.location.href="./vnr_mainindex?m='.encrypt("dashboard").'&p='.encrypt("dashboard").'";</script>';
				exit;
			}
			else
			{
				$msgErr	=	"Invalid username and password.";
			}
		}
	}
	else
...
$_POST['username']	=	"g.swapnil@vnrseeds.com";
?>
				...
				<div class="row">
					<div class="col-sm-10 col-sm-offset-1">
						<div class="login-container">
							<div class="center">
								<h4 class="blue" id="id-company-text" style="color:#008c40!important;">ADMINISTRATOR LOGIN WINDOW</h4>								
								<h1>
											...
											<form name="frm" id="frm" action="#" method="post">
												...
												<fieldset>
													...
													<div class="clearfix">
														...
														<button type="submit" class="width-35 pull-right btn btn-sm" style="background-color:#008c40!important; border-color:#008c40!important;" tabindex="<?php echo $t++;?>">
