...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	if($_POST['acn']=="save")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("insert into $tablename($fields) values($values)"))
			{
				...
				unset($flag);				
				unset($msgErr);		
				$compname	=	$dbconnection->getField("company_tbl","companyname","companyid=".$_POST['companyname']."");	
				$locname	=	$dbconnection->getField("location_tbl","locationname","locationid=".$_POST['locationname']."");					
				$rtname		=	$dbconnection->getField("rate_tbl","ratelistname","rateid=".$_POST['ratelistname']."");	
				$dbconnection->firequery("update hamaligroup_tbl set compname='".$compname."',ratename='".$rtname."',locname='".$locname."' where hgid=".$lid."");
				$_SESSION['success']	=	"Hamali group detail added successfully!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
			{
				$_SESSION['warning']	=	"Hamali group name with same company or rate list already exist. Please try again!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
		}
	...
	if($_POST['acn']=="update")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("update $tablename set $updatefields where ".$_REQUEST['pk']."=".trim(decryptvalue($_POST[$_REQUEST['pk']])).""))
			{
				...
				$locname	=	$dbconnection->getField("location_tbl","locationname","locationid=".$_POST['locationname']."");									
				$rtname		=	$dbconnection->getField("rate_tbl","ratelistname","rateid=".$_POST['ratelistname']."");	
				$dbconnection->firequery("update hamaligroup_tbl set compname='".$compname."',ratename='".$rtname."',locname='".$locname."' where hgid=".trim(decryptvalue($_POST[$_REQUEST['pk']]))."");
				$_SESSION['success']	=	"Hamali group updated successfully!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
	...
	if($_POST['acn']=="delete")
	{
		...
		{ 
			$_SESSION['success']	=	"Hamali group detail deleted successfully!";
			echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
			exit;
		}
		else
...
else
{
	...
	if($_REQUEST['pk']!="")
	{
		...
		$_POST['openingbalance']	=	doubleval($_POST['openingbalance']);
?>
...
<div class="tabbable">
		<ul class="nav nav-tabs padding-18">
			<li class="active"><a data-toggle="tab" href="#home"><i class="green ace-icon fa fa-plus-circle bigger-120" style="vertical-align:bottom;"></i> Add Hamali Group</a></li>	
			<li><a data-toggle="tab" href="#feed"><i class="orange ace-icon fa fa-list bigger-120" style="vertical-align:bottom;"></i> Hamali Group List</a></li>
		</ul>
		...
		<form name="frm" id="frm" action="#" method="post">
			...
			while($row=mysqli_fetch_assoc($rs_sel))
			{
				...
				<div class="col-sm-<?php echo $row['columnsize'];?>">
					...
					{
					?>
					<select class="form-control" name="<?php echo $row['fieldname'];?>" id="<?php echo $row['fieldid'];?>" <?php echo $row['isrequired'];?> <?php echo $row['autofocus'];?> tabindex="<?php echo $t++;?>" onKeyPress="return OnKeyPress(this, event)" <?php echo $row['onchange'];?>>
			...
			?>
				<div class="col-sm-3" style="text-align:right;">
					...
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Add Hamali Group</button>
					<?php
					}
					else
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Update Hamali Group Detail</button>
					<?php
					}
					?>
				</div>
	...
	<div class="table-responsive">
		<table class="table table-bordered table-hover" id="tablerecords">
			<thead>
				...
				<tr>
					<th style="padding:3px; text-align:center; width:80px;">Group No</th>
					<th style="padding:3px;">Company Name</th>
					<th style="padding:3px;">Group Name</th>					
					<th style="padding:3px;">Contact 1</th>
					<th style="padding:3px;">Location</th>					
					<th style="padding:3px;">Aadhar Card</th>					
					<th style="padding:3px;">Pan Card</th>					
					<th style="padding:3px;">Opening</th>					
					<th style="padding:3px;">Rate List</th>					
...
<script>
	...
	function LoadDefault()
	{
		...
		$(".loader").show();		
		$.post("ajaxpages/hamaligroup.php",
			{
				processname: "hamaligroup",
				pagesize: pagesize,
