...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	if($_POST['acn']=="save")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("insert into $tablename($fields) values($values)"))
			{
				...
				unset($flag);				
				unset($msgErr);		
				$_SESSION['success']	=	"Group member detail added successfully!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
			{
				$_SESSION['warning']	=	"Group member already exist. Please try again!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
		}
	...
	if($_POST['acn']=="update")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("update $tablename set $updatefields where ".$_REQUEST['pk']."=".trim(decryptvalue($_POST[$_REQUEST['pk']])).""))
			{
				...
				unset($row_fields);				
				unset($up);
				$_SESSION['success']	=	"Group member detail updated successfully!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
	...
	if($_POST['acn']=="delete")
	{
		...
		{ 
			$_SESSION['success']	=	"Group member detail deleted successfully!";
			echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
			exit;
		}
		else
...
<div class="tabbable">
		<ul class="nav nav-tabs padding-18">
			<li class="active"><a data-toggle="tab" href="#home"><i class="green ace-icon fa fa-plus-circle bigger-120" style="vertical-align:bottom;"></i> Add Hamali Group Member</a></li>	
			<li><a data-toggle="tab" href="#feed"><i class="orange ace-icon fa fa-list bigger-120" style="vertical-align:bottom;"></i> Hamali Group Member List</a></li>
		</ul>
		...
		<form name="frm" id="frm" action="#" method="post">
			...
			?>
				<div class="col-sm-12" style="text-align:right;">
					...
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Add Hamali Group Member</button>
					<?php
					}
					else
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Update Hamali Group Member Detail</button>
					<?php
					}
					?>
				</div>
			...
			<thead>
				<tr>
					...
					<div id="tablehead">
						...
						<select name="grpname" id="grpname" class="selectbx" onchange="LoadDefault()" tabindex="<?php echo $t++;?>">
				...
				<tr>
					...
					<th style="padding:3px;">Member Name</th>
...
<script>
	...
	function LoadDefault()
	{
		...
		$(".loader").show();		
		$.post("ajaxpages/groupmember.php",
			{
