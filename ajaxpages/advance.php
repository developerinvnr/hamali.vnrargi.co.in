...
if(isset($_SESSION['datadetail'][0]['sessionid']))
{
	...
	if($hgname=="")
	{
		$query	=	"select a.*,b.groupname from advance_tbl a left join hamaligroup_tbl b on b.hgid=a.groupname ";
		if($searchvalue!='')
		{
			$query.=	"where (a.bankname like '%$searchvalue%') ";
		}
	...
	{
		$query	=	"select a.*,b.groupname from advance_tbl a left join hamaligroup_tbl b on b.hgid=a.groupname where a.groupname=".$hgname." ";
		if($searchvalue!='')
		{
			$query.=	"and (a.bankname like '%$searchvalue%') ";
		}
		$start	=	$pagesize*($pagenumber-1);
		$query.=	" limit $start, $pagesize";
	}
...
$rs_count	=	$dbconnection->firequery("select * from advance_tbl");
...
<tr>
	...
	<td align="center"><?php echo $row['slipnumber']; $cols++;?></td>
	<td align="center"><?php echo date('d\-m\-Y h:i A',strtotime($row['advancedate'])); $cols++;?></td>	
	<td><?php echo $row['groupname']; $cols++;?></td>
	<td><?php echo $row['amount']; $cols++;?></td>	
	<td>
	<?php 
	echo $row['paymentmode'];
	if($row['cdno']!="") echo "<br>".$row['cdno'];
	if($row['bankname']!="") echo "<br>".$row['bankname'];
	?>
	</td>	
	...
	<td class="center" id="action"><a href="./printing/printvoucher.php?advanceid=<?php echo $row['advanceid'];?>" target="_blank"><i class="fa fa-print"></i></a></td>						
</tr>
<?php
}
if($j>0)
