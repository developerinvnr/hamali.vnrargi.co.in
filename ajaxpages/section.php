...
if(isset($_SESSION['datadetail'][0]['sessionid']))
{
	...
	if($searchvalue!='')
	{
		$query.=	"where (sectionname like '%$searchvalue%') ";
	}
...
<tr>
	...
	<td><?php echo $row['sectionname']; $cols++;?></td>
	<td>
	<?php 
	$rs_dep	=	$dbconnection->firequery("select * from department_tbl where departmentid in (".$row['departmentname'].")");
	while($dep=mysqli_fetch_assoc($rs_dep))
	{
		echo $dep['departmentname']."<br>";
	}
	?></td>	
