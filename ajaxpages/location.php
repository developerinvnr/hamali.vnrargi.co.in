...
if(isset($_SESSION['datadetail'][0]['sessionid']))
{
	...
	$query	=	"select * from $tablename a inner join city_tbl b on b.cityid=a.cityname ";
	if($searchvalue!='')
	{
		$query.=	"where a.locationname like '%$searchvalue%' or b.cityname like '%$searchvalue%' ";
	}
	if($orderby!='')
	{
		$exp	=	explode("-",$orderby);
...
<tr>
	...
	<td><?php echo $dbconnection->getField("city_tbl","cityname","cityid=".$row['cityid'].""); $cols++;?></td>
	<td><?php echo $row['locationname']; $cols++;?></td>
	<td class="center" id="action"><a href="./mainindex.php?m=<?php echo $_REQUEST['m'];?>&p=<?php echo $_REQUEST['p'];?>&pk=<?php echo encrypt($row[$_REQUEST['pk']]);?>"><i class="fa fa-edit"></i></a><?php  $cols++;?></td>
...
?>
<tr>
	<td colspan="8" style="padding:0px; text-align:center;">
