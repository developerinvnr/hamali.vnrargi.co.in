...
<tr style="background-color:#008C40; color:white;">
	<td colspan="4" align="center">WORK CODE WISE FOR DEPARTMENT [<?php echo $dbconnection->getField("department_tbl","departmentname","departmentid=".$departmentid."");?>]</td>
</tr>
<tr style="background-color:#008C40; color:white;">
...
$totalqty	=	0;
$totalamt	=	0;
$rs_det	=	$dbconnection->firequery("select distinct(workcode),sum(quantity) as quantity,sum(total) as total from workslip_detail a left join supervisor_tbl b on b.supervisorid=a.supervisorid where b.locationname=".$row['locationid']." and b.departmentname=".$departmentid." and date(a.creationdate) between '".$frmdate."' and '".$todate."' group by a.workcode order by a.workcode");
