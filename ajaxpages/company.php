...
if(isset($_SESSION['datadetail'][0]['sessionid']))
{
	...
	if($searchvalue!='')
	{
		$query.=	"where (companyname like '%$searchvalue%' or regnumber like '%$searchvalue%' or gstnumber like '%$searchvalue%' or address like '%$searchvalue%' or pincode like '%$searchvalue%')";
	}
...
<tr>
	...
	<td>
	<?php 
	$rs_loc	=	$dbconnection->firequery("select * from location_tbl where locationid in (".$row['locationname'].")");
	...
	<td><?php echo $row['address'];?><br /><?php echo $row['pincode'];?></td>	
