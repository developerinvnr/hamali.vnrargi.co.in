...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	if($_POST['acn']=="save")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("insert into $tablename($fields) values($values)"))
			{
				...
				unset($flag);				
				unset($msgErr);				
				$_SESSION['success']	=	"State name added successfully!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
			{
				$_SESSION['warning']	=	"State name already exist. Please try again!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
		}
	...
	if($_POST['acn']=="update")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("update $tablename set $updatefields where ".$_REQUEST['pk']."=".trim(decryptvalue($_POST[$_REQUEST['pk']])).""))
			{
				...
				unset($row_fields);				
				unset($up);				
				$_SESSION['success']	=	"State name updated successfully!";
	...
	if($_POST['acn']=="delete")
	{
		if(!$dbconnection->isRecordExist("select * from city_tbl where statename=".trim(decryptvalue($_POST[$_REQUEST['pk']])).""))
		{		
			if($dbconnection->firequery("delete from $tablename where ".$_REQUEST['pk']."=".trim(decryptvalue($_POST[$_REQUEST['pk']])).""))
			{ 
				$_SESSION['success']	=	"State name deleted successfully!";
				echo '<script>document.location.href="./mainindex.php?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
		...
		{
			$_SESSION['warning']	=	"You can not delete this state name . Since this state name is associated with some cities!";
			echo '<script>document.location.href="./mainindex.php?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
			exit;		
		}
	}
		...
		<form name="frm" id="frm" action="#" method="post">
			...
			?>
				<div class="col-sm-3">
					...
					{
					?>
					<button type="submit" class="btn" tabindex="<?php echo $t++;?>">Add State Name</button>
					<?php
					}
					else
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Update State Name</button>
					<?php
					}
					?>
				</div>
		...
		<table class="table table-bordered table-hover" id="tablerecords">
			<thead>
				...
				<tr>
					...
					{
					?> 					
					<th style="padding:3px;"><?php echo ucwords($caps['searchcaption']);?></th>
					<?php
					}
					unset($rs_caps);
					unset($caps);					
