...
if($_SERVER['REQUEST_METHOD']=="POST")
{
	if($_POST['acn']=="save")
	{
		...
		$location	=	"";
		$i=0;
		foreach($_POST['locationname'] as $key=>$val)
		{
			$i++;
			if($i==1)
			{
				$location.=	$_POST['locationname'][$key];
			}
			else
			{
				$location.=",".$_POST['locationname'][$key];
		...
		{
			$flag++;
			$msgErr.=	"<li class='text-warning'>Location name is mandatory field</li>";
		}		
		if($flag==0)
		{
			...
			if($dbconnection->firequery("insert into $tablename($fields) values($values)"))
			{
				...
				unset($flag);				
				unset($msgErr);			
				$dbconnection->firequery("update company_tbl set locationname='".$location."' where companyid=".$lid."");
				$_SESSION['success']	=	"Company detail added successfully!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
			{
				$_SESSION['warning']	=	"Company detail already exist. Please try again!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
		}
	...
	if($_POST['acn']=="update")
	{
		...
		if($flag==0)
		{
			...
			if($dbconnection->firequery("update $tablename set $updatefields where ".$_REQUEST['pk']."=".trim(decryptvalue($_POST[$_REQUEST['pk']])).""))
			{
				...
				unset($row_fields);				
				unset($up);
				$dbconnection->firequery("update company_tbl set locationname='".$location."' where ".$_REQUEST['pk']."=".trim(decryptvalue($_POST[$_REQUEST['pk']]))."");
				$_SESSION['success']	=	"Company detail updated successfully!";
				echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
				exit;
			}
			else
	...
	if($_POST['acn']=="delete")
	{
		...
		{ 
			$_SESSION['success']	=	"Company detail deleted successfully!";
			echo '<script>document.location.href="./vnr_mainindex?m='.$_REQUEST['m'].'&p='.$_REQUEST['p'].'";</script>';
			exit;
		}
		else
...
<div class="tabbable">
		<ul class="nav nav-tabs padding-18">
			<li class="active"><a data-toggle="tab" href="#home"><i class="green ace-icon fa fa-plus-circle bigger-120" style="vertical-align:bottom;"></i> Add Company</a></li>	
			<li><a data-toggle="tab" href="#feed"><i class="orange ace-icon fa fa-list bigger-120" style="vertical-align:bottom;"></i> Company List</a></li>
		</ul>
		...
		<form name="frm" id="frm" action="#" method="post">
			...
			while($row=mysqli_fetch_assoc($rs_sel))
			{
				...
				<div class="col-sm-<?php echo $row['columnsize'];?>">
					...
					<?php
						...
						{
							$exp	=	explode(",",$_POST['locationname']);
						}					
			...
			?>
				<div class="col-sm-3">
					...
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Add Company Detail</button>
					<?php
					}
					else
					{
					?>
					<button type="submit" class="btn btn-info" tabindex="<?php echo $t++;?>">Update Company Detail</button>
					<?php
					}
					?>
				</div>
		...
		<table class="table table-bordered table-hover" id="tablerecords">
			<thead>
				...
				<tr>
					...
					<th style="padding:3px;">Registration Number</th>					
					<th style="padding:3px;">GST Number</th>										
					<th style="padding:3px;">Location</th>					
					<th style="padding:3px;">Address Detail</th>										
...
<script>
	...
	function ShowLocation(cityname,htmlid)
	{
	  var xhttp;    
	  xhttp = new XMLHttpRequest();
	  xhttp.onreadystatechange = function() {
	  ...
	  xhttp.open("POST", "ajaxpages/locationname.php?q="+cityname, true);
	  xhttp.send();
	}
